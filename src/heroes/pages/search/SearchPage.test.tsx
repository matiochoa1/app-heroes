...
describe("SearchPage", () => {
  ...
  test("should call searchAction with name parameter", () => {
  ...
  test("should call search action with strength and name parameter", () => {
    renderSearchPage(["/search?name=batman&strength=6"]);
  ...
  test("should render hero grid with search results", async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText("Clark Kent"));
      expect(screen.getByText("Bruce Wayne"));
    });
  });
});
